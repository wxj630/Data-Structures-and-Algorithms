...
class logistic_regression():
    ...
    def logistic(self, X, y, W, b):
        num_train = X.shape[0] # 几行
        num_feature = X.shape[1] # 几列

        a = self.sigmoid(np.dot(X, W) + b) # sigema(z)
    ...
    def logistic_train(self, X, y, learning_rate, epochs):
        ...
        for i in range(epochs): # 迭代训练
            a, cost, dW, db = self.logistic(X, y, W, b) # 计算当前次的模型计算结果、损失核参数梯度
            W = W - learning_rate * dW # 参数更新
            b = b - learning_rate * db
            if i % 100 == 0: # 记录损失
                cost_list.append(cost)
            if i % 100 == 0: # 打印训练过程中的损失
    ...
    def plot_logistic(self, X_train, y_train, params):
        ...
        x = np.arange(0, 0.2, 0.1) # 起点，终点，步长
...
if __name__ == "__main__":
    ...
    model.plot_logistic(X_test, y_test, params)